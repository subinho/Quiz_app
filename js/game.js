document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleQuestion = () => {
    ...
    counter++;
    if (progress) progress.innerHTML = `${counter} / ${questions.length}`;
    const index = Math.floor(Math.random() * availableQuestions.length);
    currentQuestion = availableQuestions[index];
    if (question) question.innerHTML = currentQuestion.question;

    options.forEach((option) => {
      const dataset = option.dataset['option'];
      if (option) option.innerHTML = currentQuestion['option' + dataset];
    });
